...
public class Person {
    ...
    public static void createPeopleById(String name, String sex, String dateStr) throws ParseException {
        ...
        if (sex.equals("м")) {
            Solution.allPeople.add(Person.createMale(name, date.parse(dateStr)));
        } else {
            Solution.allPeople.add(Person.createFemale(name, date.parse(dateStr)));
        }
        System.out.print(Solution.allPeople.size() - 1 + " ");
    }

    public static void updatePeopleById(String id, String name, String sex, String dateStr) throws ParseException {
        SimpleDateFormat date = new SimpleDateFormat("dd/MM/y");
        Person iP = Solution.allPeople.get(Integer.parseInt(id));
        iP.setName(name);
    ...
    public static void nullPeopleById(int id){
        Person iP = Solution.allPeople.get(id);
