...
public class Solution {
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 1; i < 11; i++)    //очередь из 10 задач
        {
            final int temp = i;
            Runnable runnable = new Runnable() {
                @Override
                public void run() {
                    doExpensiveOperation(temp); //реализация выполнения
                }
            };
            blockingQueue.add(runnable);   //помещаем задачу в очередь
        }
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(3, 5, 1000, TimeUnit.MILLISECONDS, blockingQueue); // п.4
        threadPoolExecutor.prestartAllCoreThreads();
        threadPoolExecutor.shutdown();              //Запрет на добавление новых задач на исполнение в пул
        threadPoolExecutor.awaitTermination(5, TimeUnit.SECONDS);    // 5 секунд на исполнение
