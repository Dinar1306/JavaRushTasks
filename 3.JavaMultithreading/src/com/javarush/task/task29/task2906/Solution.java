...
public class Solution {
    ...
    public static Integer getValue(boolean first, boolean second) {
        return first ? Integer.valueOf(100) : second ? 200 : null; // было просто 100 -
        /*проще всего заменить первый примитив на обертку, т.е. Integer.valueOf(100) - далее всё проходит на ура
        смысл в том, чтоб избежать автораспаковки для левого тернарного оператора - т.е. нужно чтоб оба значения были обертками,
        тогда и результат тернарного оператора тоже будет оберткой*/
    }
}
