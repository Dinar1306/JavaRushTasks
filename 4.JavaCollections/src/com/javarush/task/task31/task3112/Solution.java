...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        Path passwords = downloadFile("https://javarush.ru/testdata/secretPasswords.txt", Paths.get("D:/SLIFE"));
    ...
    public static Path downloadFile(String urlString, Path downloadDirectory) throws IOException {
        ...
        URL url = new URL(urlString);                                       // объект URL
        InputStream inputStream = url.openStream();                         //поток для скачивания (скачивание)

        Path tempFile = Files.createTempFile("temp-",".tmp");  // временный файл
        Files.copy(inputStream, tempFile, StandardCopyOption.REPLACE_EXISTING);           // из потока в файл

        String name = urlString.substring(urlString.lastIndexOf("/"));  // имя файла по ссылке
        if (Files.notExists(downloadDirectory)) Files.createDirectories(downloadDirectory); // папка скачивания

        Path destPath=Paths.get(downloadDirectory.toString(), name);        // полный путь папки скачивания (с новым именем файла)
        Files.move(tempFile, destPath, StandardCopyOption.REPLACE_EXISTING);// перемещение+переименование и перезапись если уже есть
