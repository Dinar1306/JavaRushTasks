...
public class Solution {
    ...
    public static Object convertOneToAnother(Object one, Class resultClassObject) throws IOException {
        ...
        String st = one.getClass().getSimpleName(); //получаем имя класса из объекта
        StringWriter writer = new StringWriter(); // готовим читатель строк
        mapper.writeValue(writer, one);           // сериализация полученного в параметрах объекта one
        StringReader reader = new StringReader(writer.toString().replaceFirst(one.getClass().getSimpleName().toLowerCase(), resultClassObject.getSimpleName().toLowerCase())); // готовим ридер строки с передачей полученного JSON(команда writer.toString())
        o = mapper.readValue(reader, resultClassObject); // десериализация
        return o; // mapper.readValue(reader, resultClassObject); - без Object
    }
