...
Историки добавили несколько дат произошедших событий в уникальное множество в методе initializeDates. Далее они точнее
изучили исторические материалы, и уточнили время последнего события lastDate. Каково же было их изумление, когда в этом
множестве не находится нужная дата - метод isLastDateInDates возвращает false.
Амиго - твой долг помочь истории. Внеси необходимые изменения, чтобы дата последнего события находилась в множестве
dates (чтобы вывод программы был true).
...
public class Solution {
    ...
    private void initializeDates() {
        ...
        //Set unmodifiableSet = Collections.unmodifiableSet(dates);
    }

    protected void updateLastDate(long delta) {
        Date tempDate = lastDate;
        dates.remove(lastDate);
        lastDate.setTime(tempDate.getTime() + delta);
        dates.add(lastDate);
    }
}